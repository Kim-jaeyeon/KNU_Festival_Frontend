...
const BoothAndFoodTruck: React.FC = () => {
  ...
  const prevLocation = () => {
    ...
    setActiveLocation(locations[(idx - 1 + locations.length) % locations.length]);
    setPage(1);
  };
  const nextLocation = () => {
    const idx = locations.indexOf(activeLocation);
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      >
        <div className="relative w-[353px] h-[251px] flex justify-center items-center rounded-[10px] bg-[rgba(255,255,255,0.8)] shadow-md px-[10px] pt-[12px] pr-[9px] pb-[11px]">
          ...
          >
            <img src="/assets/booth/icons/arrow-left.svg" alt="prev" width={26} height={26} />
          </button>
          ...
          >
            <img src="/assets/booth/icons/arrow-right.svg" alt="next" width={26} height={26} />
          </button>
      ...
      {/* 상단 버튼 영역 */}
      <div className="flex justify-center gap-[4px] flex-wrap mt-3">
        {locations.map((loc) => (
          <button
            key={loc}
            onClick={() => {
      ...
      {/* 페이지네비게이션 */}
      <div className="flex justify-center items-center gap-[22px] mt-[25px]">
        {/* 이전 버튼 */}
        <button
          disabled={page === 1}
